...
CHECKPOINT_PATH = Path("./multistream_checkpoints/best_model.pt")
RESULTS_DIR = Path("./multistream_results")
MODEL_EXPORT_DIR = Path("./multistream_model_export")
...
def _prepare_model(ckpt: Dict, label_map: Dict[str, int], device: torch.device) -> MultiStreamClassifier:
    label_map = ckpt.get("label_map", label_map)
    cfg = ckpt.get("model_config", {})
    model = MultiStreamClassifier(
        num_classes=len(label_map),
        max_len=int(ckpt.get("max_len", 1)),
        gcn_hidden=int(cfg.get("gcn_hidden", 64)),
        gcn_out=int(cfg.get("gcn_out", 96)),
        d_model=int(cfg.get("d_model", 192)),
        conformer_layers=int(cfg.get("conformer_layers", 4)),
        conformer_heads=int(cfg.get("conformer_heads", 4)),
        conformer_ff=int(cfg.get("conformer_ff", 256)),
        dropout=float(cfg.get("dropout", 0.2)),
        proj_dim=int(cfg.get("proj_dim", 128)),
        use_cosine=bool(cfg.get("use_cosine", False)),
        cosine_scale=float(cfg.get("cosine_scale", 30.0)),
    )
...
def main() -> int:
    ...
    export_path = export_dir / "arsl_multistream.pt"
    export_payload = {
    ...
    config = {
        "architecture": "MultiStreamConformer",
        "max_len": max_len,
        "num_classes": len(label_map),
        "model_config": ckpt.get("model_config", {}),
        "label_map": label_map,
