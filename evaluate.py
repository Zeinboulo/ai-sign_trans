...
MODEL_EXPORT_DIR = Path("./model_export")
...
def _resolve_paths() -> Tuple[Path, Path, Path, Path, Path, Path]:
    if FILTERED_MANIFEST_PATH.exists() and FILTERED_LABEL_MAP_PATH.exists():
        return (
            ...
            Path("./checkpoints_filtered/best_model.pt"),
            Path("./results_filtered"),
            Path("./model_export_filtered"),
        )
    return (
        MANIFEST_PATH,
        LABEL_MAP_PATH,
...
def _prepare_model(ckpt: Dict, label_map: Dict[str, int], device: torch.device) -> CNNBiLSTM:
    label_map = ckpt.get("label_map", label_map)
    model = CNNBiLSTM(
        num_classes=len(label_map),
        input_dim=int(ckpt.get("input_dim", 225)),
        use_projection=bool(ckpt.get("use_projection", True)),
        projection_dim=int(ckpt.get("projection_dim", 512)),
    )
...
def main() -> int:
    ...
    export_path = export_dir / "arsl_cnn_bilstm.pt"
    export_payload = {
