...
if DATASET == "fewshot_4_19":
    ...
    CHECKPOINT_PATH = Path("./hybrid_fewshot_checkpoints_4_19/best_fewshot.pt")
    ...
    CHECKPOINT_PATH = Path("./hybrid_fewshot_checkpoints_4plus/best_fewshot.pt")
    ...
    CHECKPOINT_PATH = Path("./hybrid_fewshot_checkpoints_filtered/best_fewshot.pt")
...
def main() -> int:
    ...
    _, _, test_df = create_splits(MANIFEST_PATH, SPLITS_DIR, seed=42, force=False)
    ...
    encoder = HybridEncoder(
        ...
        tf_layers=int(cfg.get("tf_layers", 3)),
        tf_heads=int(cfg.get("tf_heads", 4)),
        tf_ff=int(cfg.get("tf_ff", 192)),
    ...
    print(f"Hybrid few-shot test accuracy: {mean_acc*100:.2f}% (std {std_acc*100:.2f}%)")
    print(f"Hybrid few-shot test loss: {mean_loss:.4f}")
